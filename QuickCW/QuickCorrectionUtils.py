...
def reflect_cosines(cos_in,angle_in,rotfac=np.pi,modfac=2*np.pi):
    ...
    :param modfac:
    angle to change azimuthal angle by before getting back to the same point [2*np.pi]
...
def reflect_into_range(x, x_low, x_high):
    ...
    :param x:
    Value of parameter to reflect in range
    :param x_low:
    Lower bound of parameter
    :param x_high:
    Upper bound of prameter
...
def check_merged(log10_fgw,log10_mc,max_toa):
    ...
    :param log10_fgw:
    log base 10 of GW frequency
    :param log10_mc:
    log base 10 of chirp mass
    :param max_toa:
    Latest TOA in any pulsar in the array
...
def correct_extrinsic_array(samples,x0):
    ...
    :param samples:
    2D array with parameters for multiple samples
    :param x0:
    CWInfo object
...
def correct_intrinsic(sample,x0,freq_bounds,cut_par_ids, cut_lows, cut_highs):
    ...
    :param cut_highs:
    Upper bounds of parameters needing extra check
